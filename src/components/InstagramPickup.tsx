...
export function InstagramPickup({ onAdded }: { onAdded: (events: OutingEvent[]) => void }) {
  ...
  function submit(e: React.FormEvent) {
    ...
    setUrl(""); setTitle(""); setVenueName(""); setError(""); setOpen(false);
  }

  return (
    <section className="mb-5">
      <button type="button" onClick={() => setOpen((v) => !v)} className="w-full rounded-3xl px-4 py-3 text-left text-sm" style={{ background: "var(--chip)" }}>
        インスタから拾う {open ? "▲" : "▼"}
      </button>
      {open && (
        <div className="mt-3 rounded-3xl p-4" style={{ background: "var(--bg-elev)", border: "1px solid var(--line)" }}>
          <p className="mb-3 text-sm" style={{ color: "var(--muted)" }}>
            投稿を開いてリンクをコピーし、ここに貼って予定へ入れます。自動取得はしません。
          </p>
          ...
          <form onSubmit={submit} className="space-y-3">
            <input value={url} onChange={(e) => setUrl(e.target.value)} placeholder="https://www.instagram.com/p/…" className="w-full rounded-2xl px-3 py-2 text-sm" style={{ background: "var(--chip)", outline: "none" }} />
            ...
            <input value={title} onChange={(e) => setTitle(e.target.value)} placeholder="タイトル（例: 水上ビルマルシェ）" className="w-full rounded-2xl px-3 py-2 text-sm" style={{ background: "var(--chip)", outline: "none" }} />
            <input value={venueName} onChange={(e) => setVenueName(e.target.value)} placeholder="場所（任意）" className="w-full rounded-2xl px-3 py-2 text-sm" style={{ background: "var(--chip)", outline: "none" }} />
            ...
            <button type="submit" className="w-full rounded-2xl py-3 text-sm font-semibold" style={{ background: "var(--accent)", color: "#fffaf1" }}>予定に拾う</button>
          </form>
        </div>
      )}
    </section>
